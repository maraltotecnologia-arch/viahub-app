...
const tipoConfig: Record<string, { icon: typeof Info; colorClass: string }> = {
  ...
  warning: { icon: AlertTriangle, colorClass: "text-yellow-500" },
  manutencao: { icon: Wrench, colorClass: "text-orange-500" },
  cobranca: { icon: CreditCard, colorClass: "text-green-500" },
};
...
function NotificacaoItem({ n, onRead }: { n: Notificacao; onRead: () => void }) {
  ...
  return (
    <button
      onClick={onRead}
      className="w-full text-left flex items-start gap-3 p-3 hover:bg-muted/50 rounded-md transition-colors"
    >
