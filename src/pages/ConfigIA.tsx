...
import { Sparkles, Brain, Plane, Hotel, Map, MessageCircle, Save } from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-fade-in-up max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center justify-center h-10 w-10 rounded-xl bg-gradient-to-br from-primary to-secondary text-primary-foreground shadow-lg">
          <Sparkles className="h-5 w-5" />
        </div>
        <div>
          <h2 className="text-2xl font-bold text-foreground">Inteligência Artificial</h2>
          <p className="text-sm text-muted-foreground">Configure o comportamento do ViaHub AI Copilot</p>
        </div>
      </div>

      {/* Beta banner */}
      <div className="rounded-xl p-4 flex items-center gap-3 border border-primary/20 bg-primary/5 dark:bg-primary/10">
        <Brain className="h-5 w-5 shrink-0 text-primary" />
        <p className="text-sm text-muted-foreground">
          <strong className="text-primary">ViaHub AI</strong> está em fase Beta. As configurações abaixo prepararão o comportamento do copiloto quando a integração estiver ativa.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="rounded-lg border p-4 bg-muted/30 space-y-3">
              <div className="flex items-center gap-2">
                <div className="flex items-center justify-center h-8 w-8 rounded-lg bg-blue-500/15 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  max={100}
                  value={markupVoos}
                  onChange={(e) => setMarkupVoos(e.target.value)}
                  className="w-full"
                />
            ...
            <div className="rounded-lg border p-4 bg-muted/30 space-y-3">
              <div className="flex items-center gap-2">
                <div className="flex items-center justify-center h-8 w-8 rounded-lg bg-amber-500/15 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  max={100}
                  value={markupHoteis}
                  onChange={(e) => setMarkupHoteis(e.target.value)}
                  className="w-full"
                />
                <span className="text-sm font-medium text-muted-foreground shrink-0">%</span>
              </div>
            ...
            <div className="rounded-lg border p-4 bg-muted/30 space-y-3">
              <div className="flex items-center gap-2">
                <div className="flex items-center justify-center h-8 w-8 rounded-lg bg-emerald-500/15 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  min={0}
                  max={100}
                  value={markupPasseios}
                  onChange={(e) => setMarkupPasseios(e.target.value)}
                  className="w-full"
                />
                <span className="text-sm font-medium text-muted-foreground shrink-0">%</span>
              </div>
